...
export function registerNotesHandlers(): void {
  ...
  // Get note by ID
  createHandler('notes:getById', 'getting note', async (noteId: string) =>
    notesService.getNoteById(noteId)
  );

  // Get all notes
  createHandler('notes:getAll', 'getting all notes', async () =>
  ...
  // Delete note
  createHandler('notes:delete', 'deleting note', async (noteId: string) =>
    notesService.deleteNote(noteId)
  );

  // Search notes
  createHandler('notes:search', 'searching notes', async (query: string) =>
    notesService.searchNotes(query)
  );

  // Get all tags
  createHandler('notes:getAllTags', 'getting tags', async () => {
